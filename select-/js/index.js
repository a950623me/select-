...
DropList.prototype = {
  ...
  init: function(options) {
    ...
    // })
    document
      .getElementsByClassName("inputElem")[0]
  ...
  _renderHTML: function(targetVal, targetParent, elemHeight) {
    ...
    if (_config.dataSource.length > 0) {
      for (var i = 0, ilen = _config.dataSource.length; i < ilen; i += 1) {
        if (_config.dataSource[i].text.indexOf(targetVal) == 0&&targetVal!="") {
          html +=
            ...
            '">' +'<span style="color:red">'+targetVal+'</span>'
           +_config.dataSource[i].text.split(targetVal)[1]+
            "</li>";
            console.log( _config.dataSource[i].text.split(targetVal));
        } else if(_config.dataSource[i].text.indexOf(targetVal)<0){
          $("ul", targetParent).css({ border: "none" });
        }else if(_config.dataSource[i].text.indexOf(targetVal)>=0&&targetVal==""){
             html +=
            '<li class="dropmenu-item p-index' +
            i +
            '" data-value="' +
